...
average_sales_per_product = (
    df.groupby("ProductName")["PricePerUnit"].mean().reset_index()
)
print("\nAverage Sales Per Product:")
...
df_filtered = df_filtered.withColumn(
    "SaleDate", col("SaleDate").cast("date")
)  # Casting if not automatically inferred
...
# Perform aggregations
# Total sales per region
total_sales_per_region = df_filtered.groupBy("Region").agg(
    sum("PricePerUnit").alias("TotalSales")
)
print("\nTotal Sales Per Region:")
total_sales_per_region.show()

# Average sales per product
average_sales_per_product = df_filtered.groupBy("ProductName").agg(
    avg("PricePerUnit").alias("AveragePrice")
)
print("\nAverage Sales Per Product:")
