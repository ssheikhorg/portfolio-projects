        ...
        for rules in repo["rules_sets"]:
            ...
            for rule in rules["rules"]:
                ...
                if rule["rule_name"] in logic_hash_list.values():
                    ...
                    num_rule_name = list(logic_hash_list.values()).count(
                        rule["rule_name"]
                    )
                ...
                if logic_hash in logic_hash_list and not is_private_rule:
                    logging.info(
                        "Skipping rule '%s > %s' because it has the same logic hash as '%s'",
                        repo["name"],
                        rule["rule_name"],
                        logic_hash_list[logic_hash],
                    )
                    continue
                ...
                rule["metadata"] = align_yara_rule_reference(
                    rule["metadata"], repo["url"]
                )

                # Modify the rule date
                rule["metadata"] = align_yara_rule_date(
                    rule["metadata"], repo["repo_path"], rules["file_path"]
                )
                ...
                rule["metadata"] = align_yara_rule_description(
                    rule["metadata"], repo["name"]
                )

                # Modify the rule author
                rule["metadata"] = align_yara_rule_author(
                    rule["metadata"], repo["author"]
                )
                ...
                custom_importance_score = retrieve_custom_importance_score(
                    repo["name"], rules["file_path"], rule["rule_name"]
                )
                if custom_importance_score:
                    modify_meta_data_value(
                        rule["metadata"], "importance", custom_importance_score
                    )
                    logging.debug(
                        "Custom importance score for rule %s is %d",
                        rule["rule_name"],
                        custom_importance_score,
                    )
                ...
                private_rules_used = check_rule_uses_private_rules(
                    rule_set_id, rule, private_rule_mapping
                )
                if private_rules_used:
                    ...
                    rule["condition_terms"] = adjust_identifier_names(
                        rule_set_id, rule["condition_terms"], private_rules_used
                    )
                ...
                # Add license URL
                modify_meta_data_value(
                    rule["metadata"], "license_url", repo["license_url"]
                )

                # Sort the meta data values
                rule["metadata"] = sort_meta_data_values(
                    rule["metadata"], YARA_FORGE_CONFIG
                )
...
def add_tags_to_rule(rule):
    ...
    tag_names = [
        ...
        "mitre_attack_techniques" "attack_technique",
        "attack_techniques",
        "attack",
        "attacks",
        "attack_type",
    ...
    rule["metadata"] = modify_meta_data_value(
        rule["metadata"], "tags", ", ".join(tags_to_add)
    )
...
def retrieve_custom_importance_score(repo_name, file_path, rule_name):
    ...
    with open("yara-forge-custom-scoring.yml", "r", encoding="utf-8") as f:
        ...
        logging.debug(
            "Checking custom importance score for rule %s in file %s in repo %s",
            rule_name,
            file_path,
            repo_name,
        )
            ...
            for rule_field, rule_value in importance_score["rule"].items():
                if rule_field == "name":
                    if rule_name.startswith(rule_value):
                        logging.debug(
                            "Rule name %s starts with %s", rule_name, rule_value
                        )
                    ...
                    if file_path.endswith(rule_value):
                        logging.debug(
                            "File path %s ends with %s", file_path, rule_value
                        )
                        rule_elements_matched = True
                    else:
                        rule_elements_matched = False
...
def sort_meta_data_values(rule_meta_data, YARA_FORGE_CONFIG):
    ...
    rule_meta_data.sort(
        key=lambda x: (
            fixed_order.index(list(x.keys())[0])
            if list(x.keys())[0] in fixed_order
...
def adjust_identifier_names(repo_name, condition_terms, private_rules_used):
    ...
    for private_rule in private_rules_used:
        ...
        for i, condition_term in enumerate(condition_terms):
            ...
            if (
                condition_term == private_rule["old_name"]
                and private_rule["repo"] == repo_name
            ):
...
def check_rule_uses_private_rules(repo_name, rule, ext_private_rule_mapping):
    ...
    for private_rule in ext_private_rule_mapping:
        ...
        if (
            private_rule["old_name"] in rule["condition_terms"]
            and private_rule["repo"] == repo_name
        ):
...
def align_yara_rule_description(rule_meta_data, repo_description):
    ...
    description_value = (
        f"No description has been set in the source file - {repo_description}"
    )
    # We create a copy so that we can delete elements from the original
    ...
    for meta_data in meta_data_copy:
        for key, value in meta_data.items():
            ...
            # If the value starts with one of the prefixes, then we found the description
                ...
                tuple(description_values_prefixes)
            ):
                description_found = True
                description_value = value
                # Remove the description from the original meta data
...
def evaluate_yara_rule_score(rule, YARA_FORGE_CONFIG):
    ...
    if meta_data_rule_score > 0:
        logging.debug(
            "Rule '%s' has a meta data score of %d",
            rule["rule_name"],
    ...
    for meta_data in rule["metadata"]:
        for field, value in meta_data.items():
            ...
            if isinstance(value, str) and any(
                keyword in value.lower() for keyword in meta_data_keywords_suspicious
            ):
                return 65
            if isinstance(value, str) and any(
                keyword in value.lower() for keyword in meta_data_keywords_hunting
            ):
                return 50
        ...
        for key, value in meta_data.items():
            ...
            if key in uuid_names:
                ...
                if is_valid_uuidv5(value):
                    ...
                    logging.debug(
                        "The value '%s' is not a valid UUID. Using our UUID instead "
...
def align_yara_rule_date(rule_meta_data, repo_path, file_path):
    ...
    if file_path in date_lookup_cache:
        ...
        (git_creation_date, git_modification_date) = get_rule_age_git(
            repo_path, file_path
        )
        ...
        for key, value in meta_data.items():
            ...
            if key in date_names:
                ...
                # If the date cannot be parsed, we removed the field, because it could cause confusion
                    ...
                    logging.debug(
                        "The date '%s' could not be parsed. Removing the field.", value
                    )
                    rule_meta_data.remove(meta_data)
...
def get_rule_age_git(repo_path, file_path):
    ...
    if commits:
        ...
        logging.debug(
            "Retrieved date info for file %s from git log. "
